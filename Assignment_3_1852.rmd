...
ridgeless = function(train, test, eps = 1e-10){
  ...
  pca_train = prcomp(Xtrain, center = TRUE, scale = FALSE)
  F1_train = Xtrain %*% pca_train$rotation
  
  pca_test = prcomp(Xtest, center = TRUE, scale = FALSE)
  F1_test = Xtest %*% pca_test$rotation
  
  regression_train = solve(t(F1_train)%*%F1_train)%*%t(F1_train)%*%t(Ytrain)
  
  Ytrain.hat = F1_train%*%regression_train
  Ytest.hat = F1_test%*%regression_train
