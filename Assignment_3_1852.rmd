...
for(i in 1:6){
  ...
  plot(timeseries, main=paste("Timeseries",i), xlim = )  # 52 length vectors
}
...
# 52 by 9 Design Matrix


#F = #natural cubic spline 1:52 - converted from the time one
  # we do not not care about intercept so need to remove the column mean
# dim(F[,0])
  
  #coefficients is DF minus 1
  # imagine F is X and X is Y
  #just like how you get coefficients for linear regression

#X is the response not the predictor
#811 responses = X1 Y1to Y 811
# each row is a a reponse, what we want to predict, not input of the ns 
#input is just time
#811 time series
#predictor is T = 1:52 
#use T to predict X. each row of X put on Y axis and T you put on X axis
# for each of these scatter plots build a natural cubic spline to make a prediction on each row of X matrix
...
#Remove column mean F

# F = F - colMeans(F)
# dim(F)
# 
# B_tp = (t(F)%*%F)^(-1)%*%t(F)#*t(X)
# print(dim(B_tp))
# 
# B = t(B_tp)
