...
for (i in 1:6){
  ...
  for (j in 1:length(cluster)){
    val = cluster[j]
    weekly_sales = c(weekly_sales, c(X[j,]))
    week_numbers = c(week_numbers, c(1:52))
  }
  
  df = data.frame(week_number = week_numbers, timeseries = weekly_sales)
  
  if(i==1){
    plot1 = ggplot(df, aes(week_number, timeseries)) + geom_smooth()
  }
  if(i==2){
    plot2 = ggplot(df, aes(week_number, timeseries)) + geom_smooth()
  }
  if(i==3){
    plot3 = ggplot(df, aes(week_number, timeseries)) + geom_smooth()
  }
  if(i==4){
    plot4 = ggplot(df, aes(week_number, timeseries)) + geom_smooth()
  }
  if(i==5){
    plot5 = ggplot(df, aes(week_number, timeseries)) + geom_smooth()
  }
  if(i==6){
    plot6 = ggplot(df, aes(week_number, timeseries)) + geom_smooth()
  }
}



ggarrange(plot1,plot2,plot3,plot4,plot5,plot6, labels = c("Plot1","Plot2","Plot3","Plot4","Plot5", "Plot6"), ncol = 3, nrow = 2)

# for(i in 1:6){
#   timeseries = F%*%centers[i,]
#   #ggplot(timeseries, main=paste("Timeseries",i))  # 52 length vectors
#   ggplot(timeseries, aes(x="Weeks",y="Weekly Sales"))
# }


## gg plot
## check the plot for mean zero and if it does not , make sure it does
#scale will look like the scale 
## -6 to 6 because it plotting just mean
## 3 sigma error and band will 

#for loop and adds one regression 

#plot.ts
#for loop and add 1 to each layer
#for loop each time plot X against y yhat onto that plot
...
ridgeless = function(train, test, eps = 1e-10){
  ...
  # pca_train = prcomp(Xtrain, center = TRUE, scale = FALSE, tol = eps)
  ...
  # print(dim(solve(t(F1_train)%*%F1_train)))
  # print(dim(t(F1_train)))
  # print(dim(t(Ytrain)))
  regression_train = t(F1_train) %*% Ytrain/colSums(F1_train ^ 2)
  # regression_train = solve(t(F1_train)%*%F1_train)%*%t(F1_train)%*%Ytrain
...
```{r}
ridgeless2 = function(train, test, eps = 1e-10){
  ...
  pca_res = prcomp(Xtrain, center = TRUE, scale = FALSE, tol =eps)
  new_Xtrain = predict(pca_res,newdata = Xtrain)
  
  intercept_ = mean(Ytrain)
  A_hat = t(new_Xtrain) %*% Ytrain/colSums(new_Xtrain ^ 2)
  Ytrain.hat = new_Xtrain %*% A_hat + intercept_
  
  new_Xtest = predict(pca_res,newdata=Xtest)
  
  Ytest.hat = new_Xtest %*% A_hat + intercept_
...
```{r}
full_model_mspe = function(myData, i, all.test.id){
